...
%package client-libs
Requires: /usr/share/java/log4j.jar
Group: System Environment/Daemons
Summary: Jars required for GUMS Clients
...
# Separate out client jars from the rest
( cd $RPM_BUILD_ROOT%{_noarchlib}/%{dirname}
  ls *.jar
) > ALL_JARS
  
( cd $RPM_BUILD_ROOT%{_noarchlib}/%{dirname}
  ls gums-client-*.jar gums-core-*.jar *log4j*.jar slf4j*.jar
) > CLIENT_JARS
  
fgrep -vxf CLIENT_JARS ALL_JARS > OTHER_JARS
sed -i s,^,%{_noarchlib}/%{dirname}/, CLIENT_JARS OTHER_JARS
# rpmbuild will interpret [xyz].jar as a glob...
sed -i 's/[][]/?/g' CLIENT_JARS OTHER_JARS
...
%files client-libs -f CLIENT_JARS
